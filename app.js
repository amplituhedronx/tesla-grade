...
const state = {
  unit: localStorage.getItem("grade-unit") || "m", mode: "gps",
  watchId: null, pollId: null, simId: null, tickId: null,
  lastPos: null, lastAlt: null, lastClimbSample: 0, lastFixT: 0,
  climbLog: [], profile: [], smoothAlt: null, vs: 0, grade: 0, vsReady: false,
...
function advanceOdo(now, speed, lat, lon) {
  ...
  if (lat != null && lon != null && state.lastLat != null && dt > 0 && dt < 8) {
    ...
    if (step >= 0.25 && step < 250) { state.odo += step; if (spd < 0.4) spd = step / Math.max(dt, 0.4); }
...
function drawIncline() {
  ...
  const wedge = $("inclineWedge"); const bar = $("inclineBar");
  if (!wedge || !bar) return;
  ...
  wedge.setAttribute("points", (cx - hx) + "," + (cy - hy) + " " + (cx + hx) + "," + (cy + hy) + " " + (cx + hx) + "," + cy + " " + (cx - hx) + "," + cy);
...
function drawProfile() {
  const lineEl = $("profileLine"), fillEl = $("profileFill"), dotEl = $("profileDot"), hint = $("profileHint");
  if (!lineEl || !fillEl || !dotEl) return;
  const padL = 70, padT = 16, iw = 910, ih = 172;
  const pts = state.profile.slice();
  if (pts.length === 1) pts.push({ t: pts[0].t + 1000, alt: pts[0].alt, dist: pts[0].dist + 1 });
  if (!pts.length) {
    lineEl.setAttribute("d", ""); fillEl.setAttribute("d", ""); dotEl.setAttribute("cx", "-20");
    if (hint) hint.textContent = "Drive or tap Sim";
    $("profileX0").textContent = fmtDist(0); $("profileX1").textContent = fmtDist(0); $("profileX2").textContent = fmtDist(0);
    $("profileRange").textContent = fmtDist(0); return;
  }
  ...
  const d0 = pts[0].dist, d1 = pts[pts.length - 1].dist, dspan = Math.max(d1 - d0, 0), collapsed = dspan < 5;
  function xy(p, i) {
    const x = collapsed ? padL + (i / Math.max(pts.length - 1, 1)) * iw : padL + ((p.dist - d0) / Math.max(dspan, 1)) * iw;
    const y = padT + (1 - (toDisp(p.alt) - min) / span) * ih;
    return [x, y];
  }
  let line = "";
  for (let i = 0; i < pts.length; i++) { const pt = xy(pts[i], i); line += (i ? " L" : "M") + pt[0].toFixed(1) + " " + pt[1].toFixed(1); }
  const last = xy(pts[pts.length - 1], pts.length - 1);
  let area = line;
  for (let i = pts.length - 1; i >= 0; i--) { const pt = xy(pts[i], i); area += " L" + pt[0].toFixed(1) + " " + Math.min(padT + ih, pt[1] + 16).toFixed(1); }
  area += " Z";
  fillEl.setAttribute("d", area); lineEl.setAttribute("d", line);
  dotEl.setAttribute("cx", last[0].toFixed(1)); dotEl.setAttribute("cy", last[1].toFixed(1));
  ...
  $("profileRange").textContent = fmtDist(dspan);
}
...
function stepSim(sim, dt) {
  ...
  sim.alt += speed * (seg.grade / 100) * dt; sim.lat += (speed * dt) / 111320;
...
function onError(err) {
  ...
  if (code === 1) { setStatus("off", "Location blocked"); showGate("Location blocked", "Allow location, then tap Enable location."); }
...
if (isTesla) startWatch();
  ...
  showGate("Enable location", isAppleTouch ? "Tap Enable location, then Allow \u2014 or use Sim." : "Tap to allow GPS, or switch to Sim.");
}
